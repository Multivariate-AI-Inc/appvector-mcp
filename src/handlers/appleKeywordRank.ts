...
export class AppleKeywordRankHandler extends BaseHandler {
  async handle(args: any): Promise<any> {
    ...
    try {
      const result = await this.fetchApi('/ranks/apple/', queryParams);
      ...
      throw new Error(`Failed to fetch Apple keyword ranks: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}